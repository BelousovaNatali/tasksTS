...
const normalizeData = (unnormalizedData: Array<Data>): Result => {
    const {byId, allIds}: Result = {
        byId: {},
        allIds: []
    }

    const groups = _(unnormalizedData).groupBy('id');
    for (let el of _(groups).keys()) {
        byId[el] = groups[el].length ? groups[el][0] : {};
        allIds.push(el);
    }
...
console.dir(normalizeData(posts));
/**
 * {
 *    byId: {
